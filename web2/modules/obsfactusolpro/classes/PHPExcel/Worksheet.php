...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
    ...
    */
     ...
     * @return PHPExcel_Worksheet
     */
 	public function fromArray($source = null, $nullValue = null, $startCell = 'A1', $strictNullComparison = false, $stringColumns = array()) {
        if (is_array($source)) {
                ...
                foreach($rowData as $cellValue) {
                    if ($strictNullComparison) {
                        if ($cellValue !== $nullValue) {
                            // Set cell value
                            if(count($stringColumns) AND in_array($currentColumn, $stringColumns))
                            	$this->getCell($currentColumn . $startRow)->setValueExplicit((string) $cellValue, PHPExcel_Cell_DataType::TYPE_STRING);
                            else
